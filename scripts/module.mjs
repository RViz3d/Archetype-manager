...
/**
 * Add "Archetypes" icon button to actor sheet window title bar.
 *
 * Hooks into getActorSheetHeaderButtons to add a compact header button that
 * opens the Archetype Manager. The button only appears on sheets for actors
 * that have at least one class item.
 */
Hooks.on('getActorSheetHeaderButtons', (app, buttons) => {
  ...
  buttons.unshift({
    class: 'archetype-manager-header-btn',
    icon: 'fas fa-hat-wizard',
    label: 'Archetypes',
    onclick: () => ArchetypeManager.open(actor)
  });

  debugLog(`${MODULE_TITLE} | Archetypes header button added for ${actor.name}`);
});
